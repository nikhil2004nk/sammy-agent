...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error: apiError } = await apiClient('/auth/register', {
      method: 'POST',
    ...
    <div className="flex items-center justify-center min-h-screen bg-black text-white px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-zinc-950/80 backdrop-blur-xl rounded-2xl shadow-2xl p-10 border border-white/10 z-10 relative">
        <div className="mb-10 text-center">
          ...
          <p className="text-zinc-400 mt-2 text-sm font-medium">Get started with Sammy</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5" suppressHydrationWarning>
          <div className="space-y-1.5">
            <label className="block text-sm font-semibold text-zinc-300">Name</label>
            <input 
              type="text" 
              required 
              value={name}
